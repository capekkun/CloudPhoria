...
## 14. Git Reset and Admin-Page Merge Conflict Cleanup (Historical, No Schema Impact)

No database/schema changes in this note — it documents a code-level incident so other Kiro instances aren't confused by git history. The local repo was hard-reset to `origin/master`, and that commit itself contained an unresolved bad merge on 24 files under `Admin/` (literal `<<<<<<<`/`=======`/`>>>>>>>` markers across `AuditLogs`, `Challenges`, `Dashboard`, `InstructorApprovals`, `Notifications`, `Profile`, `Reports`, `Users`). All conflicts were resolved in favour of the version consistent with this document's Challenges/Exams/BossFights sections (Section 10, "Challenges — Fixed"), not the teammate's independently-built alternative, because the alternative's `Dashboard.aspx.cs` still queried for FunRooms pending-review data — `FunRooms` remains a real table (Section 12 area / Section G) but its Admin review UI was intentionally removed and must stay removed.

Three Admin pages that existed only on the discarded side were deleted (not a schema change, just page/code removal): `Admin/BossFights.aspx`, `Admin/FunRoomReviews.aspx`, `Admin/LearningContent.aspx`. `FunRoomQuestions`/`FunRoomReviews`-adjacent tables are unaffected — they still exist and are still used by the Student-facing Fun Room battle flow described in Section G/H; only the removed pages were an *Admin moderation UI* for them that duplicated/violated other rules.

`CloudPhoria.csproj` was de-duplicated (`<Compile Include>` had 4 Admin pages listed twice, plus 3 dangling references to the deleted pages) and rebuilt clean (0 errors) after fixing one missing `using System.Web.UI.WebControls;` in `Admin/InstructorApprovals.aspx.cs`. No table, column, or constraint was touched. If you're reading this because a build broke after a merge, check for literal `<<<<<<<` markers first (`grep -r "<<<<<<<"`) before assuming a schema issue.
